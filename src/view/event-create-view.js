...
function createEventTypeListTemplate(eventTypes, id) {
  return (
    `<div class="event__type-list">
      <fieldset class="event__type-group">
        <legend class="visually-hidden">Event type</legend>
        ${eventTypes.map((type) => createTypeTemplate(type, id)).join('')}
      </fieldset>
    </div>`
  );
}
...
function createOfferTemplate(offer, id) {
  const {title, price} = offer;
  const offerHtmlTitle = getHtmlSafeString(offer.title);

  return (
    `<div class="event__offer-selector">
      <input class="event__offer-checkbox  visually-hidden" id="event-offer-${offerHtmlTitle}-${id}" type="checkbox" name="event-offer-${offerHtmlTitle}">
...
function createOfferListTemplate(offersPack = {}, id) {
  return offersPack.offers.length !== 0 ? (
    `<section class="event__section  event__section--offers">
      <h3 class="event__section-title  event__section-title--offers">Offers</h3>
      <div class="event__available-offers">
        ${offersPack.offers.map((offer) => createOfferTemplate(offer, id)).join('')}
      </div>
    </section>`
  ) : '';
}
      ...
      <form class="event event--edit" action="#" method="post">
        <header class="event__header">
          ...
          <div class="event__field-group  event__field-group--destination">
            ...
            <input class="event__input  event__input--destination" id="event-destination-${id}" type="text" name="event-destination" value="${currentDestination ? currentDestination.name : ''}" list="destination-list-1">
        ...
        <section class="event__details">
          ${createOfferListTemplate(currentOffersPack, id)}
